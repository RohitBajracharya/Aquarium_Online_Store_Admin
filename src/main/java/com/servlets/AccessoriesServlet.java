...
public class AccessoriesServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Accessories accessories = new Accessories(productCategory, productName, productPrice, productStock,productUnit, productImageBytes);

		if (accessoriesDao.isAccessoriesNameExists(productName)) {
			accessoriesDao.updateAccessoriesStock(productName, productStock);
			((ClassPathXmlApplicationContext) context).close();
			response.sendRedirect("stock.jsp");
		} else {
			int r = accessoriesDao.insert(accessories);
			if(r>0) {
				((ClassPathXmlApplicationContext) context).close();
				response.sendRedirect("stock.jsp");
			}
